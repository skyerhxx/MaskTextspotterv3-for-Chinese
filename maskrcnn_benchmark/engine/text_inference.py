    ...
    for _, batch in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            if cfg.MODEL.SEG_ON:
                ...
                #predictions 是 list, len为2
                #prediction[0]是global_predictions, prediction[1]是char_predictions
                #proposals 是 list
                #seg_results_dict 是 dict
    ...
    if (not cfg.MODEL.TRAIN_DETECTION_ONLY):
        for image_path, prediction in predictions.items():
            ...
            if vis:
                ...
                img.save(os.path.join(visu_dir, im_name[:-3]+'png'))
...
def inference(
    ...
    if False:
        ...
        #output_folder是./output/mixtrain/inference/icdar_2015_test        
        if output_folder:
            torch.save(predictions, predictions_path)#trained_model_1000_predictions.pth
            #predictions是dict
            #dict_keys(['datasets/icdar2015/test_images/3.jpg', 'datasets/icdar2015/test_images/2.jpg', 'datasets/icdar2015/test_images/1.jpg'])

            torch.save(seg_predictions, seg_predictions_path)#trained_model_1000_seg_predictions.pth
